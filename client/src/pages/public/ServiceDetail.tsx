...
export default function ServiceDetail() {
  ...
  if (!data) return <div className="py-20 text-center text-gray-400">Serviço não encontrado.</div>;
  const s = data;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 md:px-6">
      <Link to="/services" className="mb-6 flex items-center gap-1.5 text-sm font-semibold text-gray-500 hover:text-primary-600">
        <ArrowLeft className="h-4 w-4" /> Serviços
      </Link>
      {s.image && (
        <div className="mb-8 overflow-hidden rounded-2xl aspect-video">
          <img src={s.image} alt={s.sname || s.name} className="h-full w-full object-cover" />
        </div>
      )}
      ...
      <div className="my-5 flex items-center gap-4">
        ...
        {s.duration && (
          <span className="flex items-center gap-1.5 text-sm text-gray-500">
            <Clock className="h-4 w-4" /> {s.duration} minutos
          </span>
        )}
      </div>
      {s.description && (
        <p className="text-gray-600 dark:text-slate-300 leading-relaxed">{s.description}</p>
      )}
