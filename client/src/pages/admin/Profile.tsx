  ...
  return (
    <div className="flex flex-col gap-6 max-w-xl">
      ...
      <Card>
        ...
        <CardBody>
          <form onSubmit={(e) => { e.preventDefault(); updateProfile.mutate(); }} className="flex flex-col gap-4">
            <Input label="Nome" value={form.name} onChange={(e)=>setForm(f=>({...f,name:e.target.value}))} required />
            <Input label="Email" value={user?.email||''} disabled />
            <div className="flex items-center gap-3">
              <Button type="submit" icon={<Save className="h-4 w-4" />} loading={updateProfile.isPending}>Guardar</Button>
        ...
        <CardBody>
          <form onSubmit={(e) => { e.preventDefault(); changePwd.mutate(); }} className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {pwdMsg && <span className={`text-sm ${pwdMsg.includes('Erro')||pwdMsg.includes('Senha')?'text-red-500':'text-green-600'}`}>{pwdMsg}</span>}
            </div>
          </form>
        </CardBody>
      </Card>
